...
router.get('/dashboard', ensureAuthenticated,async(req,res)=>{
       const shortUrls = await ShortUrl.find({user:req.user.email});
        ...
        res.render('index',{shortUrls:shortUrls,alreadyExists:false,user:req.user.name});
    });


router.post('/shortUrls', ensureAuthenticated,async(req,res)=>{
    const url=await ShortUrl.findOne({full:req.body.fullurl,user:req.user.email});
    if(url){
    const shortUrls = await ShortUrl.find({user:req.user.email});
        res.render('index',{shortUrls:shortUrls,alreadyExists:true,user:req.user.name});
    }
    else{
...
router.get('/:shortUrl', ensureAuthenticated, async(req,res)=>{
    const shortUrl=await ShortUrl.findOne({short:req.params.shortUrl,user:req.user.email});
